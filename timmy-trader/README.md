...
- Organizes the native GUI around Timmy's operating model: Universe, Scanner, Strategy, Execution, Broker, and Audit.
...
For normal setup, open Timmy and use the `Setup` tab. Add the Webull OpenAPI app key, app secret, default account, region, endpoint, and live switches there, then run `Verify Profile`. Timmy does not enable `Save Profile` until the current values pass Webull verification. After saving, run `Broker Check`. Timmy stores that profile locally and keeps execution on `Paper` until the selected account has been checked.
